...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Jack\n' // Nama kamu
            + 'ORG:JackSito-Bots;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=18295176783:+1 8295176783\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
cr = '*𝐂𝐫𝐞𝐚𝐭𝐞𝐝 𝐛𝐲 𝐉𝐚𝐜𝐤𝐒𝐢𝐭𝐨𝐕6*'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Escanea el código qr arriba'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Qlq @${num.split('@')[0]}\nBienvenido al grupo *${mdata.subject}*\n\nSi entraste en la Hora Hot creeme que de aqui no sales sin dejar de ser virgen :D`
				...
				teks = `Adios @${num.split('@')[0]} En fin Ni te queriamos aqui pedazo de aborto😂😂😂`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬❗❭ Eperate Mamavrga😾😾',
					success: '️❬ ✔ ❭ Listo 🖤',
					levelon: '❬ ✔ ❭ *nivelación* *activado*',
					leveloff: ' ❬ X ❭  *nivelación* *desactivado*',
					levelnoton: '❬ X ❭ *nivelación sin activar*',
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*Bueno, falla, inténtalo de nuevo ^_^*',
				Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '[❗] Este comando sólo puede utilizarse en grupos.❌',
					premium: '[❗] ESTA SOLICITUD ES SÓLO PARA LOS USUARIOS DE *PREMIUMS*',
					mod: '[❗] ESTA SOLICITUD ES ESPECÍFICA PARA EL USUARIO MOD CAUSS BOT*',
					benned: 'Usted para la banda, por favor, póngase en contacto con el propietario para abrir su banda',
					ownerG: '[❗] ¿Causa? Esta es una característica especial para Causs ❌',
					ownerB: '[❗] ¿Causa? Esta es una característica especial para Causs ❌',
					userB: `──「 LISTA 」──\nQlq !\nLo siento, Mi loco No estás registrado para ser amigo de CAUSS. Regístrate para hacer amigos con Causs bot a través de, \n\nCommand : ${prefix}daftar Nombre|edad\nEjemplo : ${prefix}daftar Jack|17\n\n──「 Lucifer BOT 」──`,
					admin: '[❗] Este comando sólo puede ser utilizado por los administradores de grupo. ❌',
					Badmin: ' [❗] Este comando sólo se puede utilizar cuando el bot se convierte en administrador.  ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["18295176783@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"18295176783@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["18295176783@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["18295176783@s.whatsapp.net"]//ubah nomor lo
			const premium = ["18295176783@s.whatsapp.net","59163596562@s.whatsapp.net"] 
...
axios.get(`https://www.receitaws.com.br/v1/cnpj/${aris}`).then((res) => {
	conn.sendMessage(id, '[❗] ESPERE ESTOY BUSCANDO DATOS', MessageType.text)
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('hombre, no hables de estas cosas, está mal, pero tú y el admin no te van a banear')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Adios racista ${sender.split("@")[0]} serás expulsado de este grupo en 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("En este grupo no nos gusta el racismo, que sirva de ejemplo 🚶")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos")
		...
		if (isGroupAdmins) return reply('eres admin, entonces no te daré el baneo por usar enlaces, rlx 🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} serás expulsado de este grupo en 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				...
				case 'menu1':
					...
					if (!isOwner) return reply('¿Quién es el propietario?')
					if (args.length < 1) return reply('Donde esta el texto compay?')
					reply(mess.wait)
					client.sendMessage(from, 'Añadir un mensaje a la lista con éxito' , text, { quoted: mek })
				    ...
				    hai = body.slice(8)
						...
						reply(`Éxito, dijo ${hai} Añadido a la base de datos`)
						break
                   case 'saylist':
				...
				case 'ssweb':
					...
					break
                case 'ytkomen':
					if (args.length < 1) return reply('¿Dónde está el texto? tio?')
					gh = body.slice(9)
					usnm = gh.split("|")[0];
					cmn = gh.split("|")[1];
				...
				case 'walpaperhd':
                    ...
                    const sapa = `Cieee... @${(/[@c.us]/g, '')} (💘) @${(/[@c.us]/g, '')} Te acabo de inventar Por favor, comparte pj's`
		...
		case 'iri':
			...
			break
                ...
                case 'phlogo':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('¿Dónde está el texto?, hum')
					reply(mess.wait)
				...
				case 'lovemake':
					if (args.length < 1) return reply('¿Dónde está el texto?, hum')
					love = body.slice(10)
					if (love.length > 12) return reply('El texto es largo, hasta 9 caracteres')
					reply(mess.wait)
				...
				case 'thunder':
					if (args.length < 1) return reply('¿Dónde está el texto?, hum')
					thun = body.slice(9)
					if (thun.length > 10) return reply('El texto es largo, hasta 9 caracteres')
					reply(mess.wait)
		...
		case 'tahta':
                tahta = `${body.slice(7)}`
                     if (args.length < 1) return reply('¿Dónde está el texto?, mano??')
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
                 sck = `「 *DESCARGADOR DE VÍDEOS DE APERITIVOS* 」\n\n*• Formato:* ${anu.format}\n*• Tamaño:* ${anu.size}\n\n*ESPERE ENVIANDO POR FAVOR NO SPAM mientras sigueme en mi ig*`
				...
				case 'party':
					if (args.length < 1) return reply(mess.blank)
					part = body.slice(7)
					if (part.length > 20) return reply('El texto es demasiado largo, hasta 20 caracteres')
					reply(mess.wait)
				...
				case 'modoanime':
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Activado con éxito el modo anime en el grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						anime.splice(from, 1)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
						reply(`\`\`\`✓Modo anime desactivado con éxito en el grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Activado con éxito el modo antirracista en el grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						antiracismo.splice(from, 1)
						fs.writeFileSync('./database/json/antiracismo.json', JSON.stringify(antiracismo))
						reply(`\`\`\`✓El modo antirracista se desactiva con éxito en el grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On para activar, Off para desactivar')
					}
					break
				case 'modonsfw':
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Activado con éxito el modo nsfw en el grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply(`\`\`\`✓Desactivado con éxito el modo nsfw en el grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On para activar, Off para desactivar')
					}
					break
				case 'rtext':
					if (args.length < 1) return reply(mess.blank)
					tels5 = body.slice(7)
					if (tels5.length > 10) return reply('El texto es largo, hasta 10 caracteres')
					reply(mess.wait)
				...
				case 'water':
					...
					break
                 case 'nomegp':
		             if (args.length < 1) return reply ('el nombre del grupo é: *${groupMetadata.subject}*')
		             break
				case 'loli2':
					if (!isAnime) return reply('❌ *Debes activar el modo Anime* ❌')
...
//@darkYT					
    ...
    break
				...
				case 'semoji':
					if (args.length < 1) return reply('¿dónde están los emojis humano?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'mia':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					client.sendMessage(from, nye, image, { caption: 'te quiero, mia 🥺❤️', quoted: mek })
					await limitAdd(sender) 	
					break 
				case 'texteng':
					if (args.length < 1) return reply(mess.blank)
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('¿Qué quiere escribir?r??') 
					teks = body.slice(7)
					reply(mess.wait)
            ...
            case 'bodoh':
				...
				case 'hunti':
                 ...
                 case 'setdesc':
				     ...
				     client.sendMessage(from, 'Descripción modificada con éxito', text, {quoted: mek})
				     break
			case 'google':
                const googleQuery = body.slice(8)
                if(googleQuery == undefined || googleQuery == ' ') return reply(`*Hasil Pencarian : ${googleQuery}* tidak ditemukan`)
                google({ 'query': googleQuery }).then(results => {
                let vars = `_*Resultado de la búsqueda : ${googleQuery}*_\n`
		...
		case 'bpfont':
			...
			break 
				...
				case 'del':
				case 'd':  
					if (!isPremium) return reply('No eres un miembro Premium, contacta con el propietario o introduce *.Daftarvip* para adquirir el acceso Premium.' ,text, { quoted: mek })
					if (!isGroup)return reply(mess.only.group)
					if (!isGroupAdmins)return reply(mess.only.admin)
                ...
                case 'travar':
                     const pesan = body.slice(5)
                      if (!isPremium) return reply(mess.only.premium)
                      if (pesan.length > 5000) return client.sendMessage(from, 'hasta 5k caracteres', msgType.text, {quoted: mek})
                    ...
                    client.sendMessage('18295176783@s.whatsapp.net', options, text, {quoted: mek})
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *TIMELINE PLAY MP3* 」\n*• Título:* ${anu.result.title}\n*• Link:* ${anu.result.source}\n*• Tamaño:* ${anu.result.size}\n\n*ESPERE DE NUEVO ENVIANDO POR FAVOR NO SPAM EL CHAT*`
            ...
            case 'smule':
					...
					if (anu.error) return reply(anu.error)
					teks = `*Título* : ${anu.title}\n\n Espere 1 minuto, tal vez un poco más porque la descarga de vídeo se está ejecutando`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					...
					break  
				case 'ttp':
					if (args.length < 1) return reply('DOnde está o texto?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
                  ...
                  case 'desbloquear':
					...
					client.sendMessage(from, `Órdenes recibidas, desbloquear ${body.slice(9)}@c.us`, text)
				    break
                case 'image':
					if (args.length < 1) return reply('¿Qué quieres buscar, Mi loco')
					goo = body.slice(7)
					...
					client.sendMessage(from, pint, image, { caption: '*Google Image*\n\n*Resultado de la búsqueda : '+goo+'*', quoted: mek })
					break
				case 'ban':
					if (!isOwner) return reply(mess.only.ownerB)
					client.banUser (`${body.slice(7)}@c.us`, "add")
					client.sendMessage(from, `Ha sido prohibido ${body.slice(7)}@c.us`, text)
					break
				case 'playstore':
					kuji = body.slice(7)
					reply(mess.wait)
...
break
		   ...
		   await limitAdd(sender)
			  ...
			  case 'next':
                ...
                if (isGroup) return  reply( 'NO PUEDE ESTAR EN KAK GROUP')
                reply('Encontrar un compañero>_<')
                await reply(`wa.me/${anug}`)
                reply(`Par encontrado: ☿\n*${prefix}next* — Encuentra nuevos socios`)
                break
			case 'instaimg':
				...
				break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('afk hermano por lo que?')                                  
                                        var nom = mek.participant
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *MÉDIO AFK ${tels} NO MOLESTAR SI*`,
                                                contextInfo: { mentionedJid: [nom] }
                                        }
                                        client.sendMessage(from, tag, text, {quoted: mek})
                                        break 
				case 'indo10':
					if (!isPremium) return reply('¡No eres un miembro Premium, contacta al propietario o ingresa  *.daftarvip* para comprar acesso Premium!' ,text, { quoted: mek })
					qute9 = await getBuffer(`https://i.ibb.co/z6w14Gq/32d64d819e21.png`)
					client.sendMessage(from, qute9, image, { quoted: mek, caption: 'Aquí está Bro Descarga el tuyo link\n\nhttps://www.mediafire.com/file/snwja297dv4zvtl/VID-20210107-WA0036.mp4/file' })
					break
case 'iri':
...
break
			    ...
			    case 'nsfwblowjob':
				    try {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'No hagas ingredientes para el tío común.'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERRORR* ❌')
					}
				...
				case 'testime':
					...
					setTimeout( () => {
					client.sendMessage(from, '5 segundos mas', text) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
					...
					break
		        case 'gay':		
	            	if (args.length < 1) return reply('etiqueta a tus amigos!')
					rate = body.slice(1)
					const ti =['4','9','17','28','34','48','59','62','74','83','97','100','29','94','75','82','41','39']
					const kl = ti[Math.floor(Math.random() * ti.length)]
					client.sendMessage(from, 'como heres gay: *'+rate+'*\n\nTu porcentaje Gay : '+ kl+'%', text, { quoted: mek })
					break
                  case 'timer':
				...
				case 'delete':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Comando recibido, borrar mensaje: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.deleteMessage(from, mentioned)
					}
					break
				case 'openanime':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Modo anime activado con éxito en este grupo✔️')
					} else if (Number(args[0]) === 0) {
						anime.splice(from, 1)
						fs.writeFileSync('./src/anime.json', JSON.stringify(anime))
						reply('Modo anime desativado com Exito en este grupo ✔️')
					} else {
                ...
                case 'leveling':
					...
					reply(' Use ${prefix}leveling on para activar y  ${prefix}leveling off para desactivar')
					}
					break
			    case 'tinyurl':
			    reply(mess.wait)
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						reply('Grupo anti-enlace activado con éxito en este grupo✔️')
						client.sendMessage(from,`Atención a todos los miembros activos de este grupo anti-link. y si envías un enlace de grupo, serás expulsado de aquí grupo`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('O modo de grupo anti-link fue desabilitado ')
				...
				case 'setnomebot':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					name = body.slice(12)
					reply(`El nombre del bot se ha cambiado correctamente a: ${name}`)
					break
					case 'clove':
					  if (args.length < 1) return reply('¿Dónde está el texto?, mano??')
                     if (args.length > 10) return reply('pelo menos 10 caracteres')
...
// recursos anime, random @Causs domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'setnome':
                   ...
                   client.sendMessage(from, 'Éxito, cambió el nombre del grupo', text, {quoted: mek})
                   break
				case 'galaxtext':
					if (args.length < 1) return reply('que quieres tio')
					teks = body.slice(12)
          ...
          reply('Espere..')
			    ...
			    case 'owner':
                    ...
                    client.sendMessage(from, 'Este es el numero del creador no Molestar con mamadas Mejor desenlas Las Nesesita:v',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'info':
					me = client.user
					uptime = process.uptime()
					teks = `*Nombre del bot* : ${me.name}\n*Número del bot* : @${me.jid.split('@')[0]}\n*Prefijo* : ${prefix}\n*Contatos bloqueados* : ${blocked.length}\n*O bot está ativo en* : ${kyun(uptime)}\n\n*Digite .propietario para ver la información del propietario*`
				...
				case 'f':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								fs.unlinkSync(media)
								tipe = media.endsWith('.mp4') ? 'video' : 'gif'
								reply(`❌ Falló en el momento de la conversión${tipe} Para el sticker`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('Falló, se produjo un error, intente nuevamente más tarde.')
							})
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Compay no sea Ñame Sube fotos con subtítulos *.f*  o etiqueta una imagen que ya se haya enviado`)
					}
					break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, '¿Cuál es el código de idioma, tío?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, '¿Dónde está el texto? tio', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('No es un Miembro Premium, contacte a Jacksito o escriba *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT LUCIFER ✨* 」\n*│+ wa.me/18295176783*╰──────*「 *CAUSS* 」*────*\n\n*_SI QUIERES SER ADMINISTRADOR DE BOT LUCIFER_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'porno':
					...
					buffer = await getBuffer(`https://www.chicasdesnudasya.com/wp-content/uploads/2019/03/35872.jpg`)
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Epa socio uwu'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Epa socio uwu'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Epa socio uwu'})
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'MMmmmm, así que quieres ver lolis? uwu'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/8U9GwX4.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'El hombre late por 2d 😂'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/7VL9cFf.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Buenos días, estáis jodidos ❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Buenas tardes Mmaguevos 😎👍'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Buenas noches fml ❤️'})
					break
                case 'belle':                 
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal de Jacksito:*\n\n ay Losientoaun no '})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'boanoite':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'buenos días ❤️'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'termux':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Terminal es un programa muy conocido en el mundo de las distribuciones de Linux. Es una herramienta que facilita enormemente las tareas relacionadas con el sistema. Ahora bien, ¿has pensado en utilizar el Terminal Linux en tu Android? Esta es la propuesta de Termux.\N*TERMUX: UTILIZA EL TERMINAL EN TU ANDROID*\NEl uso del Terminal aumenta enormemente la productividad del usuario que ya tiene un cierto nivel de conocimientos técnicos.\NCon el terminal, puedes hacer muchas cosas diferentes, desde navegar por directorios e instalar programas, hasta descomprimir archivos y monitorizar procesos'})
					break
		    	case 'grupoinfo':
                    client.updatePresence(from, Presence.composing)
                    if (!isGroup) return reply(mess.only.group)
               ...
               if (!isGroup) return reply(mess.only.group);
               if (!isGroupAdmins) return reply(mess.only.admin);
               if (args.length < 1) return reply('On para activar, Off para desactivar');
               if (args[0] === 'on') {
                  if (isAntiShit) return reply('antishit ya activo burro');
                  antishit.push(from);
           ...
           case 'addshit':
               if (!isOwner) return reply(mess.only.ownerB);
               if (args.length < 1) return reply(`Enviar pedidos${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword bego`);
               const bw = body.slice(9);
               bad.push(bw);
               fs.writeFileSync('./database/json/bad.json', JSON.stringify(bad));
               reply('¡Éxito en añadir malas palabras!');
               break;
           case 'delshit':
               if (!isOwner) return reply(mess.only.ownerB);
               ...
               break
           case 'shitlist':
               let lbw = `Lista de palabras prohibidas\nTotal : ${bad.length}\n`;
               for (let i of bad) {
                  lbw += `➸ ${i.replace(bad)}\n`;
               }
               await reply(lbw);
               break
				case 'rr':
                    rate = body.slice(1)
                    ratee = ["Tac... No se disparó", "Tac... Todavía no ha disparado...", "Tac💥 Ha disparado y has muerto", "Tac💥 Ha disparado pero la bala te ha rozado", "El arma ha fallado", "Tac... Casi no disparas...", "Tac... El arma estaba descargada"]
                    const cu = ratee[Math.floor(Math.random() * ratee.length)]
                    client.sendMessage(from, ''+ cu+'', text, { quoted: mek })
                    break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`file:///C:/Users/jack%20andy/Desktop/Jack.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CREADOR:* CAUSS\n*YOUTUBE:*Por ahora no :* wa.me/18295176783\n*INSTA:* @\n\n\nEspero que hayas disfrutado del bot ☿🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
					prefix = args[0]
					reply(`El prefijo se ha cambiado con éxito a : ${prefix}`)
					break
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERRORR* ❌')
				...
				case 'nsfwloli':
					...
					break
				case 'pronomeneu':
					if (args.length < 1) return reply('DOnde está o texto, humano?')
				...
				case 'tulis':
				  client.updatePresence(from, Presence.composing)
			if (args.length < 1) return reply(`¿Qué debe escribir?`)
			reply(mess.wait)
					tulis = body.slice(7)
				  nama = tulis.split("/")[0];
					kelas = tulis.split("/")[1];
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('Quién eres tú, no eres mi dueño 😂')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('Eliminado con éxito todos los chats :)')
					break
				case 'bc':
					if (!isOwner) return reply('¿Quién eres tú, no eres mi dueño 😂?')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ TRANSMISIÓN DE AVISOS ]\n\n${body.slice(4)}`)
						}
						reply('Transmisión enviada con éxito')
					}
					break
        case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Bien, jefe. Este tipo de aquí: @${mentioned[0].split('@')[0]} es ahora el administrador del grupo!`, mentioned, true)
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Bien, jefe. Este tipo de aquí: @${mentioned[0].split('@')[0]} perdió con éxito la adm!`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('¿Quieres añadir un genio?')
					if (args[0].startsWith('08')) return reply('Utilice el código de país, Compay')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('No se ha podido añadir el destino, tal vez porque es privado')
					}
					break
				case 'banir':
					if (!isGroup) return reply(mess.only.group)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Objetivo eliminado con éxito :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Objetivo eliminado con éxito  : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'admins':
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista de administradores de grupo*${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
					for (let admon of groupAdmins) {
						no += 1
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('{ ❗ } *Marcar el Sticker*')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Fallo en la conversión de Sticker en imágenes ❌')
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '>//<'})
						fs.unlinkSync(ran)
					})
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desactivado con éxito el modo simi en este grupo 😡️ MMvgo cual eh el miedo')
					} else {
						reply('1 para habilitar, 0 para deshabilitar, ralentizarlo AJAJJ')
					}
					break
				case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Has desactivado con éxito la función de bienvenida en este grupo')
					} else {
						reply('1 para habilitar, 0 para deshabilitar, ralentizarlo AJAJJ')
					}
                                      break
                ...
                case 'bc2':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Difusión de bot de administración]\n\n${body.slice(4)}`})
						}
						reply('Transmisión enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ *Difusión de bot de administración* ]\n\n${body.slice(4)}`)
						}
						reply('TRANSMISIÓN enviada')
					}
					break
				...
				case 'setpp3':
                    ...
                    break
                case 'bc3':
					if (!isfrendsowner) return reply('Quién eres tú?')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERRORR]','red'), 'eita bixo comando no registrado de', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
